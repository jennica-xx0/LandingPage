...
<div id="officialModal" class="exact-modal" aria-hidden="true" role="dialog">
    ...
    <div class="exact-modal__panel" role="document">
        <div class="exact-modal__header">
            <div class="flex items-center gap-3"><img src="{{ asset('img/barangaylogo.jpg') }}" alt="Logo" class="w-8 h-8 rounded-full object-cover"><h3 id="modalTitle" class="exact-modal__title">Add Official</h3></div>
...
<div id="viewOfficialModal" class="exact-modal" aria-hidden="true" role="dialog">
    ...
    <div class="exact-modal__panel" role="document">
        <div class="exact-modal__header">
            <div class="flex items-center gap-3"><img src="{{ asset('img/barangaylogo.jpg') }}" alt="Logo" class="w-8 h-8 rounded-full object-cover"><h3 class="exact-modal__title">View Official</h3></div>
            <button type="button" class="exact-modal__close" aria-label="Close" data-close><i class="fa-solid fa-xmark"></i></button>
        </div>
        <div class="exact-modal__body grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
            <div class="md:col-span-1">
...
<style>
    ...
    .exact-modal__panel {
        position: relative;
        /* Reverted to a wider panel to better suit the side-by-side layout */
        width: min(600px, 92%);
        background: #f3ede5;
        border-radius: 18px;
    ...
    .exact-modal__body .block {
        margin-bottom: 0; /* space-y utility handles spacing now */
    }
...
document.addEventListener('DOMContentLoaded', () => {
    function lockScroll(lock){ if(lock){ document.body.style.overflow='hidden'; } else { document.body.style.overflow=''; } }
    ...
    if (modal) {
        ...
        const resetPhotoPreview = () => { photoPreview.src = ''; photoPreview.classList.add('hidden'); photoPlaceholder.classList.remove('hidden'); };
        
        const openModal = () => { modal.classList.add('open'); lockScroll(true); setTimeout(() => lastNameEl?.focus(), 10); };
        const closeModal = () => { modal.classList.remove('open'); lockScroll(false); form?.reset(); resetPhotoPreview(); };

        window.openAddModal = () => { form.reset(); form.action = "{{ route('admin.officials.store') }}"; methodField.innerHTML = ""; modalTitle.textContent = "Add Official"; submitBtn.textContent = "ADD"; resetPhotoPreview(); openModal(); };
        window.openEditModal = (btn) => { const o = JSON.parse(btn.dataset.official); form.reset(); form.action = `/admin/officials/${o.id}`; methodField.innerHTML = `<input type="hidden" name="_method" value="PUT">`; modalTitle.textContent = "Edit Official"; submitBtn.textContent = "SAVE"; lastNameEl.value = o.last_name || ''; firstNameEl.value = o.first_name || ''; middleInitialEl.value = o.middle_initial || ''; positionEl.value = o.position || ''; displayOrderEl.value = o.display_order || 0; if (o.photo_path) { photoPreview.src = `/storage/${o.photo_path}`; photoPreview.classList.remove('hidden'); photoPlaceholder.classList.add('hidden'); } else { resetPhotoPreview(); } openModal(); };

        photoInput.addEventListener('change', () => { const file = photoInput.files[0]; if (file) { const reader = new FileReader(); reader.onload = e => { photoPreview.src = e.target.result; photoPreview.classList.remove('hidden'); photoPlaceholder.classList.add('hidden'); }; reader.readAsDataURL(file); } else { resetPhotoPreview(); } });
        
        form.addEventListener('submit', async e => { e.preventDefault(); const formData = new FormData(form); try { const res = await fetch(form.action, { method: 'POST', headers: { 'X-CSRF-TOKEN': csrfToken, 'Accept': 'application/json' }, body: formData }); if (res.ok) { location.reload(); } else if (res.status === 422) { const data = await res.json(); let msg = 'Please fix:\n'; for (const k in data.errors) { msg += `- ${data.errors[k].join(', ')}\n`; } alert(msg); } else { throw new Error('Server error'); } } catch (err) { console.error(err); alert('An error occurred.'); } });
    }
    ...
    if (viewModal) {
        ...
        window.openViewModal = (btn) => { const o = JSON.parse(btn.dataset.official); viewLastName.value = o.last_name || ''; viewFirstName.value = o.first_name || ''; viewMiddleInitial.value = o.middle_initial || ''; viewPosition.value = o.position || ''; viewPhoto.src = o.photo_path ? `/storage/${o.photo_path}` : '/img/kagawad-placeholder.jpg'; viewModal.classList.add('open'); lockScroll(true); };
    }
    ...
    document.addEventListener('click', e => {
        ...
        if (addBtn) { window.openAddModal(); }
        else if (editBtn) { window.openEditModal(editBtn); }
        else if (viewBtn) { window.openViewModal(viewBtn); }
        else if (closeBtn) {
